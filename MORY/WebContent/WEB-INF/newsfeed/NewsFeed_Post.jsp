	...
	$(document).ready(function()
	{
		...
		for(Newsfeed_NewsfeedDTO comm : comment)
		{
			...
			$("<%=commentdelete%>").click(function()
			{
				...
				{
					$(location).attr("href","commentdelete.do?comment_seq="+"<%=comm.getComment_seq()%>"+"&write_seq="+"${Newsfeed_NewsfeedDTO.write_seq}");
				}
			});
		<%
		}
		...
		for(Newsfeed_NewsfeedDTO comm : comment )
		{
			...
			$("<%=commentupdatebtn%>").click(function()
			{
				...
				hiddenField3.setAttribute("value", "${Newsfeed_NewsfeedDTO.write_seq}");
	    ...
	    for(Newsfeed_NewsfeedDTO comm : recomment )
	    {
		    ...
		    $("<%=recomentdelete%>").click(function()
			{
				...
				{
					$(location).attr("href","recommentdelete.do?reco_seq="+"<%=comm.getRecomment_seq()%>"+"&write_seq="+"${Newsfeed_NewsfeedDTO.write_seq}");
				}
			});
	    <%
	    }
					...
					<c:forEach var="comment" items="${commentList }" >
						<div id="commentlistdiv${comment.comment_seq }" name="commentlistdiv${comment.comment_seq }" style="border: 1px solid; width: 92%; padding: 5px; margin-left: 3%;">
							...
							<div style="min-height: 40px; vertical-align: middle;">
								<!-- 댓글내용 --> 
								<div style="text-align:left; width: 100%; min-height:  40px; " >
									<pre>${comment.comment_cont}</pre>					
								</div>
								<div style="">
									<!-- 수정버튼 -->
