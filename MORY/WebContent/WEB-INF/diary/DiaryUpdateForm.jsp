				...
				$("#submitBtn").click(function()
						{
							...
							else if ( $("#write_reg_dtm").val()=="" )
							{
								alert("내용이 누락됬습니다.");
								return;	//-- submit 액션 중단.
							}
							else if ($("#write_cont").val()=="" )
							{
								alert("날짜가 누락됬습니다.");
								return;	//-- submit 액션 중단.
							}
...
<body>
	...
	<div class="container" id="container">
		...
		<!-- 두번째 row  -->
		<div class="row " id="all1" name="all1" >
				...
				<div class="col-xs-12 col-md-12"  style="background-color: aqua;">
					<div class="col-xs-6 col-md-6" align="left">
						...
						<input type="text" id="diary_post_title" name="diary_post_title" class="diary_post_title" 
						 placeholder="제목을 입력하세요." style="width: 70%;" value="${diary.diary_post_title }" >
					</div>
					<div align="right" class="col-xs-6 col-md-6">
						<label style="text-align: right;  color: white; ">일시 :</label>
						<input type="text" id="write_reg_dtm" class="write_reg_dtm" name="write_reg_dtm" style="text-align: right; width: 70%; color: black; " 
						 placeholder="작성날짜를 선택해주세요." value="${diary.write_reg_dtm }" >
					</div>
				</div>
				<div>
					<textarea class="form-control" id="write_cont" class="write_cont" name="write_cont"
						style="min-height: 450px; height:100%;" placeholder="내용을 이이이입려어어역하아아세요">
						${diary.write_cont }
						</textarea>
