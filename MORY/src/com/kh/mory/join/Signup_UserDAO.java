	...
	public Signup_UserDTO userList(String user_id) throws SQLException
	{
		...
		try
		{
			String sql = 
					...
					+ "        , (SELECT USER_NIC FROM TBL_USER WHERE USER_ID = MR.ACCE_USER_ID) AS ACCE_USER_NIC"
					+ "    FROM TBL_USER U JOIN TBL_GENDER G"
					+ "    ON U.GEN_CODE = G.GEN_CODE"
					+ "    JOIN TBL_PAGE P"
					+ "    ON U.PAGE_CODE = P.PAGE_CODE"
					...
					+ "    JOIN TBL_CITY C"
					+ "    ON U.CITY_CODE = C.CITY_CODE"
					+ "    JOIN TBL_MEM_REQU MR"
					+ "    ON U.USER_ID = MR.REQU_USER_ID"
					+ "    JOIN TBL_MEM_REQU_RESULT MRR"
					+ "    ON MR.REQU_SEQ = MRR.REQU_SEQ"
					+ "    WHERE U.ACC_GRADE_CODE = 80"
					+ "    AND MR.MEM_REL_CODE = 10"
					+ "    AND MRR.RESULT_YN = 'Y'"
					+ "    AND U.USER_ID = ?";
			...
			if (rs.next())
			{
				...
				res.setAcce_user_id(rs.getString("ACCE_USER_ID"));
				res.setAcce_user_nic(rs.getString("ACCE_USER_NIC"));
			}
